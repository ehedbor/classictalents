...
class MainViewController {
    ...
    private fun addClass(wowClass: WowClass) {
        ...
        classViewPane.children += loader.load<Node>().apply {
            val expacSelected = model.selectedExpansionProperty().isEqualTo(wowClass.expansionProperty())
            val classSelected = model.selectedClassesProperty().booleanBinding(
                wowClass.expansionProperty()) { it[wowClass.expansion] === wowClass }

            visibleProperty().bind(expacSelected.and(classSelected))
    ...
    private fun makeClassButton(wowClass: WowClass) = ClassButton().apply {
        ...
        selectedProperty().bind(expacSelected.and(classSelected))
        visibleProperty().bind(expacSelected)
        managedProperty().bind(expacSelected)

        setOnAction {
            model.selectedClasses[wowClass.expansion] = wowClass
        }
    }
}